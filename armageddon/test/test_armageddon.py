...
def test_calculate_energy(planet, result):
    ...
    p = planet.calculate_energy(pd.DataFrame({'velocity': [10, 5, 1],
                                'mass': [5, 1, 1], 'angle': [1, 0.1, 0.01], 'altitude': [7000,6000, 50001], 
                                'distance': [0.1, 5, 7], 'radius': [1, 1, 1], 'time': [0.5, 1., 1.5]}))

    q = 0

    assert p['dedz'].iloc[0] == q
...
def test_analyse_outcome(planet, outcome):
    ...
    test =  planet.analyse_outcome(pd.DataFrame({'velocity': [10, 5, 1],
         'mass': [5, 1, 1], 'angle': [1, 0.1, 0.01], 'altitude': [7000,6000, 50001],
         'distance': [0.1, 5, 7], 'radius': [1, 1, 1], 'time': [0.5, 1., 1.5], 'dedz': [0, 200, 0]}))
    solution = {'outcome': 'Airburst', 'burst_peak_dedz': 200, 'burst_altitude': 6000,
                'burst_distance': 5.0, 'burst_energy': 5.6763925000000004e-11}

    assert test['outcome'] == solution['outcome']

    assert test['burst_peak_dedz'] == solution['burst_peak_dedz']
